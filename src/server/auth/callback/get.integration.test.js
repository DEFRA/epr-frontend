...
const mock = {
  ...
  signInSuccessInitialUserMetric: vi.fn(),
...
vi.mock(
  ...
  async (importOriginal) => ({
    metrics: {
      ...
      signInSuccessInitialUser: () => mock.signInSuccessInitialUserMetric(),
...
describe('/auth/callback - GET integration', async () => {
  ...
  describe('initial/non-initial user metrics', () => {
    it('records signInSuccessInitialUser metric when user is the linkedBy user', async ({
      server,
      msw
    }) => {
      const backendUrl = config.get('eprBackendUrl')
      ...
      expect(mock.signInSuccessInitialUserMetric).toHaveBeenCalledTimes(1)
    ...
    it('records signInSuccessNonInitialUser metric when user is not the linkedBy user', async ({
      ...
      expect(mock.signInSuccessInitialUserMetric).not.toHaveBeenCalled()
    })

    it('does not record initial/non-initial metrics when user has no linked organisation', async ({
      server,
      msw
    }) => {
      const backendUrl = config.get('eprBackendUrl')
