...
const logoutController = {
  ...
  handler: async (request, h) => {
    ...
    const oidcLogoutUrl = new URL(session.urls.logout)
    oidcLogoutUrl.searchParams.append('id_token_hint', session.idToken)
    oidcLogoutUrl.searchParams.append(
      'post_logout_redirect_uri',
      getRedirectUrl(request, paths.auth.logout)
    )

    return h.redirect(oidcLogoutUrl.toString())
  }
}
