...
import { exportTranslations, parseArgs } from './export-translations.js'
...
describe(parseArgs, () => {
  it('should parse --out argument', () => {
    ...
    expect(result.outputFile).toBe('custom/path.xlsx')
  })

  it('should default to translations.xlsx when --out is missing or empty', () => {
    expect(parseArgs([]).outputFile).toBe('translations.xlsx')
    expect(parseArgs(['--out']).outputFile).toBe('translations.xlsx')
  })
})
